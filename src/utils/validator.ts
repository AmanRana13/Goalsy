...
export const validateEmail = email => {
  // let re = /^([a-zA-Z0-9]+)([\_\.\+\-])?([a-zA-Z0-9+_.\-]+)*@([a-zA-Z0-9]+)?([\-])?([a-zA-Z0-9\-]+)+(\.[a-zA-z0-9]{2,})+$/
  let re = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,}$/g;
  return re.test(email);
};

export const trimText = txt => {
  let txt2 = removeEmojis(txt);
  let re = txt2?.trim()?.replace(/ /g, '');
  return re;
};
export const trimTextWithEmoji = txt => {
  let txt2 = removeEmojis(txt);
  let re = txt2?.trimLeft();
  return re;
};

export const removeStartingSpace = txt => {
  let re = txt?.trimLeft();
  return re;
};

export const removeSpaceBothSide = txt => {
  let re = txt?.trim();
  return re;
};

export const trimText1 = txt => {
  let txt2 = removeEmojis(txt);
  let re = txt2?.trim()?.replace(/\D/gi, '')?.replace(/^0+/, '');
  return re;
};
...
// Login Check
export const loginCheck = (email, password, isChecked) => {
  let showErrorMessage = '';
  if (email == '' || email.trim().length == 0) {
    showErrorMessage = constants.emptyEmail;
  } else if (password == '' || password.trim().length == 0) {
    showErrorMessage = constants.emptyPassword;
  } else if (!isChecked) {
...
//Create Post Check
export const createPostCheck = (description, imagesAndVideoslength) => {
  let showErrorMessage = '';
  if (description == '' || description.trim().length == 0) {
    showErrorMessage = constants.emptyDescription;
  } else if (description.length < 10) {
    showErrorMessage = constants.shortDescription;
  } else if (imagesAndVideoslength === 0) {
    showErrorMessage = constants.postImagesLength;
  }
  return showErrorMessage;
};

export const userSignUpCheck = (
  picture,
  name,
  email,
  password,
  ...
  if (!picture) {
    ...
    showErrorMessage = constants.shortPassword;
